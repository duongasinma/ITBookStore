...
public class UseDiscountController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            DiscountDTO dto = dao.getDiscount_byId(code);
            if(dto != null){
                boolean result = dao.checkDiscountDate(dto);
                if(result){
                    request.setAttribute("DISCOUNT", dto);
                }
                else request.setAttribute("CODE_ERR", "Code time out");
            }
            else{
                request.setAttribute("CODE_ERR", "Not Valid");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
